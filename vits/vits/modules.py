...
class WN(nn.Module):
    ...
    def __call__(self, x, x_mask, g=None, **kwargs):
        ...
        n_channels_tensor = mx.array([self.hidden_channels], mx.int32)
...
class Log(nn.Module):
    def __call__(self, x, x_mask, reverse=False, **kwargs):
        if not reverse:
            y = mx.log(mx.clip(x, a_min=1e-5)) * x_mask
            logdet = mx.sum(-y, [1, 2])
            return y, logdet
        else:
            x = mx.exp(x) * x_mask
...
class Flip(nn.Module):
    def __call__(self, x, *args, reverse=False, **kwargs):
        x = np.flip(x, 1)
        if not reverse:
            logdet = mx.zeros(x.size(0), x.dtype)
            return x, logdet
        else:
            return x
...
class ElementwiseAffine(nn.Module):
    ...
    def __call__(self, x, x_mask, reverse=False, **kwargs):
        if not reverse:
            ...
            y = y * x_mask
            logdet = mx.sum(self.logs * x_mask, [1, 2])
            return y, logdet
        else:
            x = (x - self.m) * mx.exp(-self.logs) * x_mask
            return x
...
class ResidualCouplingLayer(nn.Module):
    ...
    def __call__(self, x, x_mask, g=None, reverse=False):
        x0, x1 = mx.split(x, [self.half_channels] * 2, 1)
        h = self.pre(x0) * x_mask
        h = self.enc(h, x_mask, g=g)
        stats = self.post(h) * x_mask
        if not self.mean_only:
            m, logs = mx.split(stats, [self.half_channels] * 2, 1)
        else:
            m = stats
        ...
        if not reverse:
            ...
            x = mx.concatenate([x0, x1], 1)
            logdet = mx.sum(logs, [1, 2])
            return x, logdet
        else:
...
class ConvFlow(nn.Module):
    ...
    def __call__(self, x, x_mask, g=None, reverse=False):
        ...
        x = mx.concatenate([x0, x1], 1) * x_mask
        logdet = mx.sum(logabsdet * x_mask, [1, 2])
        if not reverse:
            return x, logdet
        else:
            return x
